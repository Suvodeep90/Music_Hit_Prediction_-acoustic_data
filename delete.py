...
class Config:
    ...
    def visualize(self,X, y, model):
        # plt.scatter(X[:, 0], X[:, 1], s=40, c=y, cmap=plt.cm.Spectral)
        # plt.show()
        self.plot_decision_boundary(lambda x:self.predict(model,x), X, y)
    ...
    def fit(self,X, y, nn_hdim, num_passes=20000, print_loss=False):
        ...
        for i in range(0, num_passes):
            ...
            # Backpropagation
            delta3 = probs
            delta3[range(num_examples), y.astype(int)] -= 1
            dW2 = (a1.T).dot(delta3)
